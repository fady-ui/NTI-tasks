            ...
            <tbody>
                @foreach ($products as $index => $product)
                  <tr>
                    ...
                    <td>{{number_format($product->price) }} <strong> EGP</strong></td>
                    <td @class([
                    ...
                    <td>
                            <form action="{{route('dashboard.products.toggle.status',['id'=>$product->id])}}" method="post">
                                @csrf
                                @method('PATCH')
                                <button value="{{$product->status}}" name="status"
                                 @class([
                                    'btn',
                                    'btn-sm',
                                    'rounded',
                                    'btn-outline-danger font-weight-bold'=> $product->status, 
                                    'btn-outline-success font-weight-bold' => !$product->status])
                                    >
                    ...
                    <a class="btn btn-outline-primary btn-sm rounded" href="{{route('dashboard.products.edit',['id'=>$product->id])}}"> Edit</a>
                           <form action="{{route('dashboard.products.distroy',['id'=>$product->id])}}" method="post" class="d-inline">
                            @csrf
                            @method('DELETE')
